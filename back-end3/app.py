...
from models import db, User, Product, Categorie
...
app.config['SQLALCHEMY_DATABASE_URI'] ='mysql://root:''@localhost/flaskdb3' #+ os.path.join(basedir, 'data.db')
...
def register_product():
    data = request.get_json()
    new_product = Product(dosage=data['dosage'], nom_com=data['nom_com'], description=data['description'], prix=data['prix'], date_fab=datetime.strptime(data['date_fab'], '%Y-%m-%d').strftime('%d/%m/%Y'), date_per=datetime.strptime(data['date_per'], '%Y-%m-%d').strftime('%d/%m/%Y'), qte_stock=data['qte_stock'], num_lot=data['num_lot'])
